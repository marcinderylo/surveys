...
public class AQuestionTemplateFactoryShould {
    ...
    @Test
    public void createSingleChoiceQuestionWithAnswers() throws
            Exception {
        // given
        whenCreatingAQuestion(singleChoiceQuestion("Single choice question?").
                withAnswers(answer("answer 01").disallowsOtherAnswers()));
    ...
    public void notCreateSingleChoiceQuestionIfAnyAnswerAllowsOtherAnswersToBeSelected()
            throws Exception {
        whenCreatingAQuestion(singleChoiceQuestion("Single choice question?").
                withAnswers(answer("answer 01").allowsOtherAnswers()));
        expectThatCreationFails();
    }

    @Test
    public void createMultipleChoiceQuestionWithAnswerThatRequiresComment()
            throws Exception {
        whenCreatingAQuestion(multipleChoiceQuestion("Multiple choice question?").
                withAnswers(answer("answer 01").requiresComment().
                disallowsOtherAnswers()));
        expectMultipleChoiceQuestionCreated("Multiple choice question?");
    }

    @Test(expectedExceptions = {AnswerWithCommentAllowingOtherAnswersException.class})
    public void notAllowAnswersRequiringCommentThatAllowOtherAnswers() throws
            Exception {
        whenCreatingAQuestion(multipleChoiceQuestion("multiple choice question?").
                withAnswers(answer("answer 01").requiresComment().
                allowsOtherAnswers()));
    ...
    @Test
    public void createMultipleChoiceQuestionWithAnswers() throws
            Exception {
        whenCreatingAQuestion(
                multipleChoiceQuestion("Multiple choice question?").withAnswers(
                answer("answer 01")));
        expectMultipleChoiceQuestionCreated("Multiple choice question?");
    }
    ...
    private void assertQuestionType(QuestionTemplate question,
            String expectedText,
            QuestionType expectedType) {
        assertEquals(question.getText(), expectedText);
        assertEquals(question.getType(), expectedType);
    }
