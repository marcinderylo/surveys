...
public class FilledSurveyTest {
    ...
    public void beforeMethod() throws Exception {
        ...
        when(publication.isFillingEnabled(any(DateTime.class))).thenReturn(
                Boolean.TRUE);
        questionMock = mock(QuestionTemplate.class);

        when(publication.getSurveyTemplate().getQuestion(QUESTION_NUMBER)).
                thenReturn(questionMock);

        when(publication.getSurveyTemplate().getQuestion(anyInt())).thenThrow(
                new IllegalArgumentException());

        when(publication.getSurveyTemplate().getQuestions()).thenReturn(Arrays.
                asList(questionMock));
    }
    ...
    public void shouldSubmitSurveyWhenAllQuestionsAreAnswered() throws Exception {
        ...
        addAnswers();
        survey = new FilledSurvey(publication, new UserDto());
        when(questionMock.isAnswered(any(AnsweredQuestion.class))).thenReturn(
                Boolean.TRUE);
        // when
        survey.submit();
        // then
    ...
    @Test(expectedExceptions = {SurveyNotFilledCompletelyException.class})
    public void cantSubmitSurveyWhenNotAllQuestionsAreAnswered() throws
            Exception {
        // given
        addAnswers();
        survey = new FilledSurvey(publication, new UserDto());
    ...
    @Test(expectedExceptions = InvalidAnswerNumbersException.class)
    ...
    @Test(expectedExceptions = IllegalArgumentException.class)
    public void cantGetAnswerForQuestionThatDoesntHaveThatAnswer() throws
            Exception {
        // given
        addAnswers();
        survey = new FilledSurvey(publication, new UserDto());
    ...
    @Test
    public void shouldReplaceSelectedAnswerWhenAnotherAnswerSelected() throws
            Exception {
        // given
    ...
    public void shouldAnswerQuestionWithComment() throws Exception {
        ...
        // then
        assertEquals(survey.getAnsweredQuestion(1).getComment(), "some comment");
    }
    ...
    private void addAnswers(Integer... answers) {
        ...
        AnsweredQuestion answeredQuestion = new AnsweredQuestion(questionMock,
                answerTemplates);
