...
/**
 * Translates java business exceptions to flex exceptions.
 */
public class BusinessExceptionTranslator implements ExceptionTranslator {
	...
	public static final Set<Class<?>> SUPPORTED_TYPES = ImmutableSet
			.<Class<?>> of(Integer.class, Boolean.class, Long.class,
	...
	private void ensureSupportedParameterType(Class<?> paramClass) {
		Validate.isTrue(SUPPORTED_TYPES.contains(paramClass),
				"Business exception's paramter type " + paramClass.getName()
						+ "is not supported");
	}
